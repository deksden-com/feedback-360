...
- Фича не считается закрытой, пока не пройдены **два независимых гейта**:
  1) code-quality checks (`pnpm checks` как preferred workspace gate; при необходимости дополнительно `pnpm test:db:full`, и `build` там, где это применимо),
...
- Запуск:
- “workspace quality gate”: `pnpm checks`
- “основной unit/contract lane”: `pnpm -r test`
- “каноничный DB integration lane”: `pnpm test:db`
- “расширенный DB regression lane”: `pnpm test:db:full`
...
2) Запустить code-quality checks (`lint` + `typecheck` + `test`; при необходимости `build`).
   - workspace preferred: `pnpm checks`
   - если меняли test topology / DB suite: дополнительно `pnpm test:db` и, при необходимости, `pnpm test:db:full`
...
- Автоматизация: GitHub workflow `.github/workflows/beta-smoke.yml` + Playwright suite `apps/web/playwright/tests/smoke/*.spec.ts`.
- Smoke использует dev-like runtime endpoints `/api/dev/seed` и `/api/dev/test-login` на `beta`, поэтому отдельный `BETA_SMOKE_USER_ID` не нужен.
- Каноничный запуск: `PLAYWRIGHT_BASE_URL=https://beta.go360go.ru pnpm --filter @feedback-360/web test:smoke:beta`.
- Для remote beta запуска держим повышенный timeout (`90s`), чтобы реальный Supabase/Vercel latency не превращался в ложный fail.
