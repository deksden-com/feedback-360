---
description: Feature catalog for EP-013-questionnaire-experience.
purpose: Read to see the slice breakdown inside the epic and navigate to individual feature plans.
status: Active
date: 2026-03-09
parent: .memory-bank/plans/epics/EP-013-questionnaire-experience/index.md
epic: EP-013
---


# EP-013 — Feature catalog
Status: Completed (2026-03-06)

- [FT-0131 Questionnaire inbox](FT-0131-questionnaire-inbox/index.md): список анкет с фильтрами и resume-draft flow. Читать, чтобы inbox отражал реальный status и быстро возвращал в работу.
- [FT-0132 Structured questionnaire fill flow](FT-0132-questionnaire-fill-flow/index.md): основная форма оценки с progress, save draft и submit. Читать, чтобы UX заполнения анкеты стал продуктовым.
- [FT-0133 Read-only and re-entry states](FT-0133-questionnaire-readonly/index.md): понятные состояния после submit и ended. Читать, чтобы пользователь не путался в immutable flows.
