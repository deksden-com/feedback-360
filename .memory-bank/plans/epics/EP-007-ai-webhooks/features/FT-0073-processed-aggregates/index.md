...
## User value
Employee/Manager видят только AI-обработанные тексты (processed/summary), а HR сохраняет доступ к raw. Это снижает риск deanonymization и удерживает прозрачность для HR.
...
- Ролевые витрины результатов:
  ...
  - `results.getHrView`: `rawText` + `processedText` + `summaryText`.
...
- Visibility:
  ...
  - HR output содержит `rawText` и обработанные поля.
...
- Проверено по intent:
  ...
  - employee/manager не получают raw, HR получает raw+processed+summary.
