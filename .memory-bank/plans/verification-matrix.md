...
- FT-0073: what=processed text aggregates storage (`ai_comment_aggregates`) + results visibility shaping (employee/manager no raw, HR raw+processed+summary); where=local + Supabase beta pooler + CLI/core integration; how=`pnpm --filter @feedback-360/api-contract lint && pnpm --filter @feedback-360/api-contract typecheck`, `pnpm --filter @feedback-360/db lint && pnpm --filter @feedback-360/db typecheck`, `pnpm --filter @feedback-360/core lint && pnpm --filter @feedback-360/core typecheck`, `pnpm --filter @feedback-360/client typecheck`, `pnpm --filter @feedback-360/cli lint && pnpm --filter @feedback-360/cli typecheck`, `pnpm --filter @feedback-360/web lint && pnpm --filter @feedback-360/web typecheck`, `set -a; source .env; set +a; pnpm db:migrate`, `set -a; source .env; set +a; pnpm --filter @feedback-360/core exec vitest run src/ft/ft-0071-ai-run.test.ts src/ft/ft-0055-results-views.test.ts src/ft/ft-0073-processed-text-visibility.test.ts --fileParallelism=false`, `set -a; source .env; set +a; pnpm --filter @feedback-360/db exec vitest run src/ft/ft-0072-ai-webhook.test.ts src/migrations/ft-0003-seed-runner.test.ts --fileParallelism=false`, `pnpm --filter @feedback-360/web exec vitest run src/app/api/webhooks/ai/route.test.ts`; quality_gate=passed; acceptance_gate=passed (`S9_campaign_completed_with_ai` + `ai.runForCampaign`: aggregate rows created, employee/manager dashboards exclude `rawText`, HR view returns `rawText+processedText+summaryText`); result=passed.
