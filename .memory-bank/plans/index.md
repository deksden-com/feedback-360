...
- [Runtime UI traceability follow-up](runtime-ui-traceability-follow-up.md) — completed slice that closed the remaining runtime enforcement gap for governed root `data-testid` selectors. Читать, если нужно понять, как теперь связаны `screen_id`, `testIdScope`, DOM roots и POM lookup.
