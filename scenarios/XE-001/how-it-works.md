...
> Важно: часть screenshots ниже снята прямо на `beta` с данными `XE-001`, а часть — это репрезентативные screenshots UI-потока анкеты из acceptance evidence. Это сделано потому, что `XE-001` в beta уже завершён, а значит ручной вход в этот run нужен для просмотра уже созданных данных и результатов, а не для повторного прохождения анкеты вживую.
...
## 2. Раннер создаёт сотрудников

На старте сценария seed и phase runner автоматически создают компанию и базовых участников.
...
- этот шаг в `XE-001` выполняется **не вручную через UI**, а детерминированным кодом раннера.

## 3. Раннер собирает оргструктуру
...
Это тоже системный шаг раннера, а не ручное действие на beta-экране.

## 4. Раннер подготавливает модель компетенций

`XE-001` работает на indicator-модели. В сценарии используются 4 компетенции, и у каждой есть по 2 индикатора.

Модель создаётся и связывается с кампанией автоматически, чтобы сценарий оставался воспроизводимым и не зависел от ручного ввода.

## 5. Раннер создаёт и стартует кампанию

После подготовки данных раннер создаёт и стартует кампанию `XE-001 Campaign`.
...
Именно с этого момента у нас появляется полноценный сценарный контур, который дальше уже можно проверять через UI и результаты.
...
## 7. Как в сценарии выглядит список анкет

Дальше сценарий переходит к заполнению анкет. Ниже — **репрезентативный** screenshot списка анкет из acceptance flow, который использует тот же продуктовый UI. Он показывает именно тот экран, через который runner проходит шаг заполнения.

![Questionnaire list](./assets/questionnaire-list-example.png)

## 8. Как в сценарии сохраняется черновик

Сначала анкета может быть сохранена как draft. Это важно для freeze semantics: первый `draft save` фиксирует кампанию. Ниже снова показан **репрезентативный** screenshot этого шага.

![Questionnaire draft](./assets/questionnaire-draft-example.png)

## 9. Как в сценарии отправляется анкета
...
- UI для submit показан на **репрезентативном** screenshot ниже.
...
## 14. Что важно про UI-часть сценария

В текущем `XE-001` UI используется прежде всего для:

- login bootstrap;
- проверки результирующих витрин;
- ручной инспекции уже созданного сценарного run.

А вот подготовка данных в шагах `2–5` сейчас выполняется раннером напрямую, а не через ручной HR flow на beta.

## 15. Как это связано с кодом сценария
...
## 16. Как вручную пройти результат сценария
